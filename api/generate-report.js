...
function scrubMoralLanguage(input) {
  if (!input || typeof input !== "string") return input;
  // Hard rule: remove moral/judgment language; keep meaning but neutralize tone.
  // This is intentionally conservative and cosmetic.
  const replacements = [
    [/\b(bad|good)\b/gi, ""],
    [/\b(should|shouldn't|shouldnt|must)\b/gi, "may"],
    [/\b(wrong|fault)\b/gi, "not ideal"],
    [/\b(lazy|dirty)\b/gi, "inconsistent"],
    [/\b(failure|failed)\b/gi, "didn't"],
    [/\b(ugly)\b/gi, "uneven"],
    [/\b(disgusting)\b/gi, "uncomfortable"],
  ];
  let out = input;
...
function computeConditionWeighting({ primaryConcern, clusters }) {
  // Deterministic weights from signal deficits (100-score) + a small boost from primary concern text.
  const pc = (primaryConcern || "").toLowerCase();

  const defs = [
    { key: "hydration_barrier", label: "Hydration & Barrier", keywords: ["dry", "dehyd", "hydr", "barrier", "tight", "flake"], clusterHints: ["core_skin", "skin_health", "hydr", "barrier"] },
    { key: "sensitivity_calm", label: "Sensitivity & Calming", keywords: ["sens", "red", "ros", "irrit", "react"], clusterHints: ["stress", "inflamm", "red", "sens"] },
    { key: "acne_clogging", label: "Acne & Congestion", keywords: ["acne", "break", "pore", "clog", "blem"], clusterHints: ["acne", "pore", "oil", "sebum", "congest"] },
    { key: "tone_pigment", label: "Tone & Pigmentation", keywords: ["spot", "pig", "melasma", "tone", "dark"], clusterHints: ["pig", "tone", "spot", "discolor"] },
    { key: "aging_structure", label: "Aging & Structure", keywords: ["aging", "wrinkle", "line", "firm", "lift"], clusterHints: ["aging", "structure", "texture", "firm"] },
  ];

  function clusterDeficitForHints(hints) {
    let total = 0;
    let drivers = [];
    for (const c of clusters || []) {
      const id = String(c.id || c.cluster_id || "").toLowerCase();
      const title = String(c.title || c.display_name || "").toLowerCase();
      const hit = hints.some((h) => id.includes(h) || title.includes(h));
      if (!hit) continue;
      const s = Number(c.score);
      if (!Number.isFinite(s)) continue;
      const deficit = Math.max(0, 100 - s);
      total += deficit;
      drivers.push({ cluster_id: c.id || c.cluster_id, title: c.title || c.display_name, score: s });
    }
    return { total, drivers };
  }

  const raw = defs.map((d) => {
    const { total, drivers } = clusterDeficitForHints(d.clusterHints);
    const boost = d.keywords.some((k) => pc.includes(k)) ? 15 : 0; // small deterministic boost
    return {
      key: d.key,
      label: d.label,
      raw: total + boost,
      boostApplied: boost > 0,
      drivers,
    };
  });

  const sum = raw.reduce((a, r) => a + (Number.isFinite(r.raw) ? r.raw : 0), 0);
  const normalized = raw
    .map((r) => ({
      condition: r.key,
      label: r.label,
      weight: sum > 0 ? Math.round((r.raw / sum) * 1000) / 1000 : 0,
      boostApplied: r.boostApplied,
...
function buildProtocolSet(id) {
  ...
  const map = {
    radiant: { id: "radiant", name: "Radiant Protocol", tier: "Basic", url: "https://www.skindoctor.ai/product-page/radiant-protocol-skincare-solution-natural-hydration-luminosity" },
    luxe: { id: "luxe", name: "Luxe Renewal", tier: "Advanced", url: "https://www.skindoctor.ai/product-page/luxe-renewal-complete-skincare-solution-advanced-firming-elevated-renewal" },
    clarite: { id: "clarite", name: "Clarite Protocol", tier: "Basic", url: "https://www.skindoctor.ai/product-page/clarite-protocol" },
    serein: { id: "serein", name: "Serein Balance", tier: "Advanced", url: "https://www.skindoctor.ai/product-page/serein-balance-advanced-skincare-deep-hydration-restored-calm-resilient-skin" },
  };
  return map[id] || null;
}
...
function recommendProtocols({ primaryConcern, clusters, conditionWeighting }) {
  ...
  const hasStrong = scores.some((s) => s < 60);

  function pickHydrationTrack() {
    // Upgrade to Luxe if aging is meaningful or any strong/moderate finding.
    const agingWeight = cw?.conditions?.find((c) => c.condition === "aging_structure")?.weight || 0;
    if (agingWeight >= 0.25 || hasStrong || hasModerate) return buildProtocolSet("luxe");
    return buildProtocolSet("radiant");
  }

  function pickSensitivityTrack() {
    // Upgrade to Serein if there is additional/moderate finding or hydration also meaningful.
    const hydrationWeight = cw?.conditions?.find((c) => c.condition === "hydration_barrier")?.weight || 0;
    if (hydrationWeight >= 0.25 || hasStrong || hasModerate) return buildProtocolSet("serein");
    return buildProtocolSet("clarite");
  }
...
function buildStructuredSections({ firstName, primaryConcern, captureQuality, confidenceHtml, clusters, areasOfFocus, conditionWeighting, protocols, agingPreviewImages, limitationsBullets }) {
  const name = firstName ? escapeHtml(firstName) : "there";
  const pc = primaryConcern ? escapeHtml(primaryConcern) : "your priorities";

  const cq = captureQuality || {};
  const cqLine = cq && typeof cq === "object"
    ? `Capture notes: ${escapeHtml(cq.summary || cq.reason || "Lighting/angle/expression can affect what the model can see.")}`
    : `Capture notes: Lighting/angle/expression can affect what the model can see.`;

  const observed = (clusters || [])
    .slice()
    .sort((a,b)=> (Number(a.score)||0)-(Number(b.score)||0))
    .slice(0, 5)
    .map((c) => `• ${escapeHtml(c.title || c.display_name || c.id || "Area")}: ${Number.isFinite(Number(c.score)) ? `${Number(c.score)}/100` : "—"}`)
    .join("<br/>");

  const aof = (areasOfFocus || [])
    .slice(0, 3)
    .map((a) => `• ${escapeHtml(a.title || a.cluster_title || a.cluster_id || "Area")}${Number.isFinite(Number(a.score)) ? ` (${Number(a.score)}/100)` : ""}`)
    .join("<br/>") || "• None flagged";

  const cw = (conditionWeighting?.conditions || [])
    .slice(0, 5)
    .map((c) => `• ${escapeHtml(c.label)}: ${(Math.round((c.weight || 0) * 100))}%`)
  ...
  const expectations = `
    You’ll typically see the most reliable change with consistency. Think in weeks, not days:
    <br/>• First 7–14 days: comfort, hydration, reduced reactivity
    <br/>• Weeks 3–6: texture and tone begin to normalize
    <br/>• 6–12+ weeks: deeper changes (firmness, fine lines) become more noticeable
  `;

  const aging = (agingPreviewImages && agingPreviewImages.length)
    ? `Aging preview was generated as a supportive visualization. It is not a prediction and can vary with lighting, expression, and lifestyle.`
    : `Aging preview is not included in this report.`;

  const limitations = (limitationsBullets && limitationsBullets.length)
    ? limitationsBullets.map((x) => `• ${escapeHtml(x)}`).join("<br/>")
    : "• This is a cosmetic estimate, not a diagnosis.<br/>• Lighting/angle can reduce visibility of finer details.";

  return [
    { n: 1, title: "Intent framing", html: `Dear ${name},<br/>This report is designed to reflect what your photo suggests today and translate it into practical, supportive next steps aligned with ${pc}.` },
    { n: 2, title: "Image context & integrity", html: `${cqLine}` },
    { n: 3, title: "Observed patterns (higher confidence)", html: observed || "—" },
...
`.trim();
    ...
    // EMAIL order: before -> areas of focus -> aging images -> reflection -> closing
        reportText = scrubMoralLanguage(reportText);
...
const structured_report_sections = buildStructuredSections({
      ...
      captureQuality,
      confidenceHtml,
      clusters: (visual_payload && visual_payload.clusters) ? visual_payload.clusters : [],
      areasOfFocus,
      conditionWeighting: canonical_payload.condition_weighting,
      protocols: protocol_recommendation,
      agingPreviewImages,
      limitationsBullets: [
        "This is a cosmetic estimate and does not constitute medical advice or a diagnosis.",
        "Lighting, angle, expression, and obstructions can change what is visible in a single photo.",
        "If you have a concerning lesion, rash, pain, or rapid change, consult a licensed clinician."
      ],
    });

    
    const structured_report_sections_for_client = structured_report_sections.map((s) => {
...
const protocolRecommendation = protocol_recommendation?.primary || null;
    const secondaryProtocolRecommendation = protocol_recommendation?.secondary || null;
    const protocolNonExclusivityClause = protocol_recommendation?.clause || null;
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Helvetica Neue', Arial, sans-serif; background-color: #F3F4F6;">
  ...
  <div style="max-width: 680px; margin: 0 auto; padding: 20px;">
    ...
    return res.status(200).json({
      ...
      protocolRecommendation: protocol_recommendation,
      condition_weighting: canonical_payload.condition_weighting || null,
      structured_report_sections: structured_report_sections_for_client,
