...
async function runDermatologyEngine({
  ...
  try {
    ...
    // Light normalization (add-only): ensure meta exists with the expected keys
    parsed.meta = parsed.meta || {};
    if (typeof parsed.meta.confidence_score_0_100 !== "number") parsed.meta.confidence_score_0_100 = 0;
    if (!parsed.meta.confidence_label) parsed.meta.confidence_label = "low";
    if (!Array.isArray(parsed.meta.limitations)) parsed.meta.limitations = [];
...
async function ensureStablePublicImageUrl(anyImage) {
  ...
  // Already a non-data URL; we may still want to stabilize if it looks ephemeral.
  if (typeof anyImage === "string" && !isDataUrl(anyImage)) {
    if (!looksEphemeralImageUrl(anyImage)) return anyImage;

    // Prefer Cloudinary because it can ingest remote URLs directly
    const viaCloudinary = await uploadToCloudinaryFile(anyImage);
    if (viaCloudinary) return viaCloudinary;

    // Blob fallback (download then upload)
    const viaBlob = await uploadToVercelBlobAny(anyImage);
    if (viaBlob) return viaBlob;

    // If we cannot stabilize, return original
    return anyImage;
  }
...
async function generateEditsFromSelfie({ photoDataUrl, prompts, size = "1024x1024" }) {
  ...
  async function oneEdit(prompt) {
    ...
    // Keep url output (fast), then we stabilize via Cloudinary/Blob later.
    form.append("response_format", "url");
    ...
    <div style="margin: 18px 0 18px 0; padding: 14px 14px 16px; border-radius: 10px; border: 1px solid #E5E7EB; background-color: #F9FAFB;">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin-top: 8px;">
        ...
        ${
          withCare20
            ? `<div>
                <img src="${withCare20}" alt="Approximate 20-year future with consistent care" style="width: 100%; border-radius: 10px; border: 1px solid #E5E7EB;" />
                <p style="font-size: 11px; color: #4B5563; margin: 6px 0 0;">~20 years – with consistent care</p>
              </div>`
            : ""
        }
...
`.trim();
    ...
    // ✅ NEW: Stabilize aging preview images to public URLs (Cloudinary/Blob)
