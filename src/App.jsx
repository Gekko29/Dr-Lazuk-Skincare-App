...
--------------------------------------- */
const AccordionSection = ({ title, subtitle, open, onToggle, children, sectionId }) => {
  return (
    <div id={sectionId} className="border border-gray-200 bg-white">
      <button
        onClick={onToggle}
      ...
      >
        ...
        <ChevronDown
          size={18}
          className={`text-gray-900 transition-transform ${open ? "rotate-180" : "rotate-0"}`}
        />
      </button>
...
/* ---------------------------------------
   Face Overlay Preview (static, non-emotional)
--------------------------------------- */
const FaceOverlayPreview = ({ src }) => {
  if (!src) return null;

  return (
    <div className="relative w-full max-w-[260px] border border-gray-200 bg-gray-50 overflow-hidden">
      <img src={src} alt="Preview" className="w-full block opacity-[0.96]" />
      {/* Static overlay */}
      <svg
      ...
      >
        {/* subtle frame */}
        <rect x="2" y="2" width="96" height="96" fill="none" stroke="rgba(17,24,39,0.28)" strokeWidth="0.6" />
        {/* radial cluster center */}
        <circle cx="50" cy="52" r="10" fill="none" stroke="rgba(17,24,39,0.22)" strokeWidth="0.8" />
        {/* points */}
        {[
          { x: 50, y: 20 },
          { x: 70, y: 30 },
          ...
          <g key={i}>
            <line x1="50" y1="52" x2={p.x} y2={p.y} stroke="rgba(17,24,39,0.16)" strokeWidth="0.6" />
            <circle cx={p.x} cy={p.y} r="1.6" fill="rgba(17,24,39,0.25)" />
          </g>
        ))}
        {/* crosshair */}
        <line x1="50" y1="8" x2="50" y2="96" stroke="rgba(17,24,39,0.08)" strokeWidth="0.6" />
        <line x1="6" y1="52" x2="96" y2="52" stroke="rgba(17,24,39,0.08)" strokeWidth="0.6" />
      </svg>

      <div className="absolute bottom-2 left-2 bg-white/85 backdrop-blur border border-gray-200 px-2 py-1 text-[11px] font-semibold text-gray-900">
        Static landmark overlay (preview)
      </div>
    </div>
  );
};
...
const DermatologyApp = () => {
  ...
  // ✅ NEW: section refs so “Jump to” always works (scrolls correctly)
  const refDetails = useRef(null);
  const refFocus = useRef(null);
  const refFuture = useRef(null);
  const refGuidance = useRef(null);
  const refObserve = useRef(null);
  const refReflection = useRef(null);
  ...
  const toggleSection = (key) => {
    setOpenSections((prev) => {
      ...
      gaEvent('results_section_toggled', { key, open: next[key] ? 1 : 0 });
      return next;
    });
  };

  const openAndScrollTo = (key, ref) => {
...
${SUPPORTIVE_FOOTER_LINE}`);
  ...
  const performAnalysis = async () => {
    ...
    try {
      ...
      // Reset gating + accordion state on fresh results
      setReflectionSeen(false);
      setOpenSections({
        details: false,
        focus: false,
  ...
  const quickActions = useMemo(() => {
    const actions = (focusItems || [])
      .map((it) => String(it?.now || "").trim())
      .filter(Boolean)
      .slice(0, 3);
    return actions;
  }, [focusItems]);
  ...
  const concernLabel = useMemo(() => {
    ...
    return map[primaryConcern] || primaryConcern || "Not specified";
  }, [primaryConcern]);
          ...
          <div className="bg-white border border-gray-200 shadow-sm p-8">
            ...
            {step === 'results' && analysisReport && (
              <div className="space-y-6">
                ...
                {/* ✅ DEFAULT SHORT SUMMARY VIEW (TOP) */}
                <div className="bg-white border-2 border-gray-900 p-6">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
                    <div className="flex-1">
                      <h4 className="text-xl font-bold text-gray-900 mb-2">Quick Summary</h4>
                      <p className="text-sm text-gray-700 mb-5">
                        A concise overview. All details below are optional and can be expanded.
                      </p>

                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="border border-gray-200 bg-gray-50 p-4">
                          <p className="text-xs text-gray-600 uppercase tracking-wider">Primary Concern</p>
                          <p className="text-base font-bold text-gray-900 mt-1">{concernLabel}</p>
                        </div>

                        <div className="border border-gray-200 bg-gray-50 p-4">
                          <p className="text-xs text-gray-600 uppercase tracking-wider">Age Range</p>
                          <p className="text-base font-bold text-gray-900 mt-1">{ageRange || "Not specified"}</p>
                        </div>

                        <div className="border border-gray-200 bg-gray-50 p-4 md:col-span-2">
                          <p className="text-xs text-gray-600 uppercase tracking-wider">Fitzpatrick (if provided)</p>
                          <p className="text-sm text-gray-900 mt-1">
                            <span className="font-bold">{analysisReport?.fitzpatrickType || "Not provided"}</span>
                            {analysisReport?.fitzpatrickSummary ? (
                              <span className="text-gray-700"> — {analysisReport.fitzpatrickSummary}</span>
                            ) : null}
                          </p>
                        </div>
                      </div>
                      ...
                      {focusItems?.length > 0 && (
                        <div className="mt-5">
                          <p className="text-sm font-bold text-gray-900 mb-2">Top Areas of Focus</p>
                          <div className="flex flex-wrap gap-2">
                            {focusItems.map((it, idx) => (
                              <span
                                key={`${it.title}-${idx}`}
                                className="inline-flex items-center border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-semibold text-gray-900"
                              >
                                {it.title}
                              </span>
                            ))}
                      ...
                      {quickActions?.length > 0 && (
                        <div className="mt-5 border border-gray-200 bg-white p-4">
                          <p className="text-sm font-bold text-gray-900 mb-2">Immediate Priorities</p>
                          <ul className="text-sm text-gray-800 space-y-2">
                            {quickActions.map((a, i) => (
                              <li key={i} className="leading-relaxed">
                                <span className="font-bold text-gray-900">•</span> {a}
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      {/* Jump controls */}
                      <div className="mt-6">
                        <p className="text-xs text-gray-600 uppercase tracking-wider mb-2">Open a section</p>
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={() => openAndScrollTo("details", refDetails)}
                          ...
                          <button
                            onClick={() => openAndScrollTo("focus", refFocus)}
                          ...
                          <button
                            onClick={() => openAndScrollTo("future", refFuture)}
                            className="px-4 py-2 bg-white text-gray-900 text-sm font-bold border border-gray-300 hover:bg-gray-50"
                            type="button"
                          >
                            Future Story Images
                          ...
                          <button
                            onClick={() => openAndScrollTo("guidance", refGuidance)}
                            className="px-4 py-2 bg-white text-gray-900 text-sm font-bold border border-gray-300 hover:bg-gray-50"
                            type="button"
                          >
                          ...
                          <button
                            onClick={() => openAndScrollTo("reflection", refReflection)}
                            className="px-4 py-2 bg-white text-gray-900 text-sm font-bold border border-gray-300 hover:bg-gray-50"
                            type="button"
                          >
                            Dr. Lazuk’s Note
                    ...
                    {/* Optional radial/overlay preview (static) */}
                    <div className="w-full md:w-[280px]">
                      <p className="text-xs text-gray-600 uppercase tracking-wider mb-2">
                        Preview
                      </p>
                      <FaceOverlayPreview src={capturedImage} />
                      <p className="text-xs text-gray-600 mt-3 leading-relaxed">
                        This is a static landmark-style overlay preview. It is intentionally calm and non-emotional.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div ref={refDetails}>
                    <AccordionSection
                      ...
                      title="Detailed Cosmetic Report"
                      subtitle="Full narrative interpretation (optional)."
                  ...
                  <div ref={refFocus}>
                    <AccordionSection
                      ...
                      subtitle="The strongest signals flagged right now (optional)."
                  ...
                  <div ref={refFuture}>
                    <AccordionSection
                      sectionId="section-future"
                      title="Future Story Images"
                      subtitle="Cosmetic projections anchored to your selfie (optional)."
                                ...
                                {!reflectionSeen && (
                                  <div className="mt-3 bg-white border border-gray-200 p-3">
                                    <p className="text-xs text-gray-700">
                                      Sharing/saving is enabled after you read Dr. Lazuk’s note below.
                                    </p>
                                  </div>
                                )}
                              </div>
                  ...
                  <div ref={refGuidance}>
                    <AccordionSection
                      sectionId="section-guidance"
                      title="Guidance"
                      subtitle="Recommended products and treatments (optional)."
                  ...
                  <div ref={refObserve}>
                    <AccordionSection
                      ...
                      title="Observe"
                      subtitle="Save this moment and revisit later (optional)."
                      open={!!openSections.observe}
                      onToggle={() => toggleSection("observe")}
                    >
                      <div className="bg-gray-50 border border-gray-200 p-6">
                        <p className="text-sm text-gray-700">
                  ...
                  <div ref={refReflection}>
                    <AccordionSection
                      sectionId="section-reflection"
                      title="A Message from Dr. Lazuk"
                      subtitle="Reading this enables sharing/saving (optional)."
